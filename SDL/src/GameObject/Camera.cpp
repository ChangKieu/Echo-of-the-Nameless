...
void Camera::Update(float deltaTime)
{
	if (m_target != nullptr)
	{
		...
		m_ViewBox.x = (m_target->GetPosition().x - SCREEN_WIDTH / 2) * m_zoom;
		m_ViewBox.y = (m_target->GetPosition().y - SCREEN_HEIDHT / 2) * m_zoom;
		...
		m_ViewBox.w = SCREEN_WIDTH * m_zoom;
		m_ViewBox.h = SCREEN_HEIDHT * m_zoom;
